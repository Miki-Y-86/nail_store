...
function dbConnect(){
    //DBへの接続準備
    $dsn = 'mysql:dbname=newnailstore;host=localhost;charset=utf8';
    $user = 'root';
    $password = 'root';
    $options = array(
...
function getMsgsAndBord($id){
    ...
    try {
        ...
        $dbh = dbConnect();
        // SQL文作成
        $sql = 'SELECT m.id AS m_id, product_id, bord_id, send_date, to_user, from_user, sale_user, buy_user, msg, b.create_date FROM message AS m RIGHT JOIN bord AS b ON b.id = m.bord_id b.delete_flg WHERE b.id = :id AND b.delete_flg = 0 ORDER BY send_date ASC';
        $data = array(':id' => $id);
        // クエリ実行
        $stmt = queryPost($dbh, $sql, $data);

...
if(!empty($_POST)){
    ...
    $email = $_POST['email'];
    //画像をアップロードし、パスを格納
    $pic = ( !empty($_FILES['pic1']['name']) ) ? uploadImg($_FILES['pic'], 'pic') : '';
    //画像をPOSTしてない（登録してない）が既にDBに登録されている場合、DBのパスを入れる（POSTされないので）
...
if( (int)$dbFormData['zip'] !== $zip){  //DBデータをint形にキャスト（方変換）して比較
    //郵便番号形式チェック
    validZip($zip, 'zip');
}
...
プロフィール画像
                <label class="area-drop <?php if(!empty($err_msg['pic'])) echo 'err'; ?>" style="height:370px;line-height:370px;">
                    <input type="hidden" name="MAZ_FILE_SIZE" value="3145728">
